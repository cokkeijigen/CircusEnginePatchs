...
namespace LoliHook {
	...
	//记录内存区间段
	std::unordered_map<int64_t, MemoryHandle*> memoryMap;
	...
	char *LoliMalloc(intptr_t handle) {
		//创建一个内存段索引，32位直接用0索引即可，因为到处可用
		int64_t range = 0;
		//64位需要保持前33位不动
		if (sizeof(intptr_t) > 4) range = handle & 0xFFFFFFFF80000000;
		...
		if (iter == memoryMap.end()) {
			mm = new MemoryHandle;
			//尝试在区间段内寻找可用的内存
			//0xfffffffff 1000000  --> 0xfffffffff ff00000
			for (int ii = 0x10; ii < 0xff; ii++) {
				char* adress;
				//32为模式 1010 0000这样去找
				if(sizeof(intptr_t) == 4) adress = (char *)(ii << 24 | ii << 16);
				else {
					//64位前33位不动，轮转数进30位，到达31位。
					int64_t hi = range | (ii) << 30;
					adress = (char *)hi;
				}
	...
	intptr_t ApiHook(const char *dllname, const char *funcname, void *newPtr) {
		...
		if (dll) {
			...
			if (adress) {
				//printf("function adress is:%I64X\n", adress);
	...
	void Make_Console() {
		...
		printf("LoliHook Console is OK!\n");
	}


	intptr_t GetApi(const char *dllname, const char *funcname) {
		auto dll = GetModuleHandleA(dllname);
		if (!dll) dll = LoadLibraryA(dllname);
		if (dll) {
			auto adress = GetProcAddress(dll, funcname);
			if (adress) return intptr_t(adress);
		}
		return 0;
	}
